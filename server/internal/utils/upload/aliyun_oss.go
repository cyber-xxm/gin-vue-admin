...
func (*AliyunOSS) UploadFile(file *multipart.FileHeader) (string, string, error) {
	...
	if err != nil {
		zap_logger.Error("function AliyunOSS.NewBucket() Failed", zap.Any("err", err.Error()))
	...
	if openError != nil {
		zap_logger.Error("function file.Open() Failed", zap.Any("err", openError.Error()))
	...
	if err != nil {
		zap_logger.Error("function formUploader.Put() Failed", zap.Any("err", err.Error()))
...
func (*AliyunOSS) DeleteFile(key string) error {
	...
	if err != nil {
		zap_logger.Error("function bucketManager.Delete() failed", zap.Any("err", err.Error()))
