...
func (s *ApiService) CreateApi(api system.SysApi) (err error) {
	if !errors.Is(s.db.Where("path = ? AND method = ?", api.Path, api.Method).First(&system.SysApi{}).Error, gorm.ErrRecordNotFound) {
...
func (s *ApiService) SyncApi() (newApis, deleteApis, ignoreApis []system.SysApi, err error) {
...
func (s *ApiService) IgnoreApi(ignoreApi system.SysIgnoreApi) (err error) {
	if ignoreApi.Flag {
		return s.db.Create(&ignoreApi).Error
	}
	return s.db.Unscoped().Delete(&ignoreApi, "path = ? AND method = ?", ignoreApi.Path, ignoreApi.Method).Error
}
...
func (s *ApiService) DeleteApi(api system.SysApi) (err error) {
	var entity system.SysApi
	err = s.db.First(&entity, "id = ?", api.ID).Error // 根据id查询api记录
...
func (s *ApiService) GetAPIInfoList(api system.SysApi, info request.PageInfo, order string, desc bool) (list interface{}, total int64, err error) {
	limit := info.PageSize
	offset := info.PageSize * (info.Page - 1)
	db := s.db.Model(&system.SysApi{})
...
func (s *ApiService) GetAllApis(authorityID uint) (apis []system.SysApi, err error) {
	parentAuthorityID, err := s.authorityService.GetParentAuthorityID(authorityID)
	if err != nil {
		return nil, err
	}
	err = s.db.Order("id desc").Find(&apis).Error
...
func (s *ApiService) GetApiById(id int) (api system.SysApi, err error) {
	err = s.db.First(&api, "id = ?", id).Error
	return
}
...
func (s *ApiService) UpdateApi(api system.SysApi) (err error) {
	var oldA system.SysApi
	err = s.db.First(&oldA, "id = ?", api.ID).Error
	if oldA.Path != api.Path || oldA.Method != api.Method {
		var duplicateApi system.SysApi
		if ferr := s.db.First(&duplicateApi, "path = ? AND method = ?", api.Path, api.Method).Error; ferr != nil {
