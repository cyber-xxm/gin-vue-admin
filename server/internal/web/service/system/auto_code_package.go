...
func NewAutoCodePackageService(db *gorm.DB) *AutoCodePackageService {
	return &AutoCodePackageService{
		db: db,
	}
}

type AutoCodePackageService struct {
	db *gorm.DB
}
...
func (s *AutoCodePackageService) Create(ctx context.Context, info *system2.SysAutoCodePackageCreate) error {
	switch {
	case info.Template == "":
	...
	if !errors.Is(s.db.Where("package_name = ? and template = ?", info.PackageName, info.Template).First(&system.SysAutoCodePackage{}).Error, gorm.ErrRecordNotFound) {
...
// @author: [piexlmax](https://github.com/piexlmax)
// @author: [SliverHorn](https://github.com/SliverHorn)
func (s *AutoCodePackageService) Delete(ctx context.Context, info request.GetById) error {
	err := s.db.WithContext(ctx).Delete(&system.SysAutoCodePackage{}, info.Uint()).Error
	if err != nil {
...
// @author: [piexlmax](https://github.com/piexlmax)
// @author: [SliverHorn](https://github.com/SliverHorn)
func (s *AutoCodePackageService) All(ctx context.Context) (entities []system.SysAutoCodePackage, err error) {
	server := make([]system.SysAutoCodePackage, 0)
	plugin := make([]system.SysAutoCodePackage, 0)
	...
	for i := 0; i < len(serverDir); i++ {
		if serverDir[i].IsDir() {
			serverPackage := system.SysAutoCodePackage{
	...
	for i := 0; i < len(pluginDir); i++ {
		if pluginDir[i].IsDir() {
			...
			pluginPackage := system.SysAutoCodePackage{
	...
	entitiesMap := make(map[string]system.SysAutoCodePackage)
	for i := 0; i < len(entities); i++ {
		entitiesMap[entities[i].PackageName] = entities[i]
	}
	createEntity := []system.SysAutoCodePackage{}
	...
	if len(createEntity) > 0 {
		err = s.db.WithContext(ctx).Create(&createEntity).Error
		if err != nil {
...
// Templates 获取所有模版文件夹
// @author: [SliverHorn](https://github.com/SliverHorn)
func (s *AutoCodePackageService) Templates(ctx context.Context) ([]string, error) {
...
func (s *AutoCodePackageService) templates(ctx context.Context, entity system.SysAutoCodePackage, info system2.AutoCode) (code map[string]string, asts map[string]ast.Ast, creates map[string]string, err error) {
			...
			for j := 0; j < len(secondDirs); j++ {
				...
				case "gen", "config", "initialize", "plugin":
					...
					for k := 0; k < len(threeDirs); k++ {
						...
						response := strings.Index(threeDirs[k].Name(), "system")
						...
						if gen != -1 {
							pluginGen := &ast.PluginGen{
								...
								Path:        filepath.Join(global.GVA_CONFIG.AutoCode.Root, global.GVA_CONFIG.AutoCode.Server, "plugin", entity.PackageName, secondDirs[j].Name(), strings.TrimSuffix(threeDirs[k].Name(), ext)),
								ImportPath:  fmt.Sprintf(`"%s/plugin/%s/system"`, global.GVA_CONFIG.AutoCode.Module, entity.PackageName),
								StructName:  info.StructName,
								PackageName: "system",
								IsNew:       true,
							}
				...
				case "system":
					...
					for k := 0; k < len(threeDirs); k++ {
						...
						hasModel := strings.Index(threeDirs[k].Name(), "system")
						if hasModel == -1 {
							return nil, nil, nil, errors.Errorf("[filpath:%s]非法模版文件!", four)
						}
						create := filepath.Join(global.GVA_CONFIG.AutoCode.Root, global.GVA_CONFIG.AutoCode.Server, "plugin", entity.PackageName, secondDirs[j].Name(), info.HumpPackageName+".go")
						if entity.Template == "package" {
							packageInitializeGorm := &ast.PackageInitializeGorm{
								...
								ImportPath:  fmt.Sprintf(`"%s/system/%s"`, global.GVA_CONFIG.AutoCode.Module, entity.PackageName),
