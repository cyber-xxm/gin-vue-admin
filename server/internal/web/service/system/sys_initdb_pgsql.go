...
func NewPgsqlInitHandler(db *gorm.DB) *PgsqlInitHandler {
...
func (h PgsqlInitHandler) WriteConfig(ctx context.Context) error {
	c, ok := ctx.Value("config").(orm.Pgsql)
	if !ok {
...
func (h PgsqlInitHandler) EnsureDB(ctx context.Context, conf *system.InitDB) (next context.Context, err error) {
