...
import (
	...
	"github.com/cyber-xxm/gin-vue-admin/internal/utils/ast"
...
func NewAutoCodeHistoryService(db *gorm.DB) *AutoCodeHistoryService {
	return &AutoCodeHistoryService{
		db:              db,
		apiService:      NewApiService(db),
		baseMenuService: NewBaseMenuService(db),
	}
}
...
func (s *AutoCodeHistoryService) Create(ctx context.Context, info system.SysAutoHistoryCreate) error {
	create := info.Create()
	err := s.db.WithContext(ctx).Create(&create).Error
	if err != nil {
...
// Author [SliverHorn](https://github.com/SliverHorn)
// Author [songzhibin97](https://github.com/songzhibin97)
func (s *AutoCodeHistoryService) First(ctx context.Context, info request.GetById) (string, error) {
	var meta string
	err := s.db.WithContext(ctx).Model(system2.SysAutoCodeHistory{}).Where("id = ?", info.ID).Pluck("request", &meta).Error
	if err != nil {
...
// Author [SliverHorn](https://github.com/SliverHorn)
// Author [songzhibin97](https://github.com/songzhibin97)
func (s *AutoCodeHistoryService) Repeat(businessDB, structName, abbreviation, Package string) bool {
	var count int64
	s.db.Model(&system2.SysAutoCodeHistory{}).Where("business_db = ? and (struct_name = ? OR abbreviation = ?) and package = ? and flag = ?", businessDB, structName, abbreviation, Package, 0).Count(&count).Debug()
	return count > 0
}
...
// Author [SliverHorn](https://github.com/SliverHorn)
// Author [songzhibin97](https://github.com/songzhibin97)
func (s *AutoCodeHistoryService) RollBack(ctx context.Context, info system.SysAutoHistoryRollBack) error {
	var history system2.SysAutoCodeHistory
	err := s.db.Where("id = ?", info.ID).First(&history).Error
	if err != nil {
		return err
	}
	if history.ExportTemplateID != 0 {
		err = s.db.Delete(&system2.SysExportTemplate{}, "id = ?", history.ExportTemplateID).Error
		if err != nil {
			return err
		}
	}
	if info.DeleteApi {
		...
		err = s.apiService.DeleteApisByIds(ids)
		if err != nil {
			zap_logger.Error("ClearTag DeleteApiByIds:", zap.Error(err))
		}
	} // 清除API表
	if info.DeleteMenu {
		err = s.baseMenuService.DeleteBaseMenu(int(history.MenuID))
		if err != nil {
	...
	err = s.db.WithContext(ctx).Model(&system2.SysAutoCodeHistory{}).Where("id = ?", info.ID).Update("flag", 1).Error
	if err != nil {
...
// Author [SliverHorn](https://github.com/SliverHorn)
// Author [songzhibin97](https://github.com/songzhibin97)
func (s *AutoCodeHistoryService) Delete(ctx context.Context, info request.GetById) error {
	err := s.db.WithContext(ctx).Where("id = ?", info.Uint()).Delete(&system2.SysAutoCodeHistory{}).Error
	if err != nil {
...
// Author [SliverHorn](https://github.com/SliverHorn)
// Author [songzhibin97](https://github.com/songzhibin97)
func (s *AutoCodeHistoryService) GetList(ctx context.Context, info request.PageInfo) (list []system2.SysAutoCodeHistory, total int64, err error) {
	var entities []system2.SysAutoCodeHistory
	db := s.db.WithContext(ctx).Model(&system2.SysAutoCodeHistory{})
...
func (s *AutoCodeHistoryService) DropTable(BusinessDb, tableName string) error {
	if BusinessDb != "" {
		return global.MustGetGlobalDBByDBName(BusinessDb).Exec("DROP TABLE " + tableName).Error
	} else {
		return s.db.Exec("DROP TABLE " + tableName).Error
	}
}
