...
func NewAutoCodeMysqlService(db *gorm.DB) *AutoCodeMysqlService {
	return &AutoCodeMysqlService{
		db: db,
	}
}

type AutoCodeMysqlService struct {
	db *gorm.DB
}
...
func (s *AutoCodeMysqlService) GetDB(businessDB string) (data []sysResp.Db, err error) {
...
// Author [piexlmax](https://github.com/piexlmax)
// Author [SliverHorn](https://github.com/SliverHorn)
func (s *AutoCodeMysqlService) GetTables(businessDB string, dbName string) (data []sysResp.Table, err error) {
...
// Author [piexlmax](https://github.com/piexlmax)
// Author [SliverHorn](https://github.com/SliverHorn)
func (s *AutoCodeMysqlService) GetColumn(businessDB string, tableName string, dbName string) (data []sysResp.Column, err error) {
...
ORDER BY 
    c.ORDINAL_POSITION;`
	if businessDB == "" {
		err = s.db.Raw(sql, tableName, dbName).Scan(&entities).Error
	} else {
