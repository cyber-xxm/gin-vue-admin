...
func (s *CasbinService) UpdateCasbin(adminAuthorityID, AuthorityID uint, casbinInfos []system.CasbinInfo) error {
	...
	if global.GVA_CONFIG.System.UseStrictAuth {
		apis, e := s.apiService.GetAllApis(adminAuthorityID)
		if e != nil {
			return e
		}
	...
	s.casbinService.ClearCasbin(0, authorityId)
	rules := [][]string{}
	//做权限去重处理
...
func (s *CasbinService) GetPolicyPathByAuthorityId(AuthorityID uint) (pathMaps []system.CasbinInfo) {
	e := s.Casbin()
	authorityId := strconv.Itoa(int(AuthorityID))
	list, _ := e.GetFilteredPolicy(0, authorityId)
	for _, v := range list {
		pathMaps = append(pathMaps, system.CasbinInfo{
			Path:   v[1],
			Method: v[2],
		})
	}
