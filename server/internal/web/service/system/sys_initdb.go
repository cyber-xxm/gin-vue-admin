...
type TypedDBInitHandler interface {
	EnsureDB(ctx context.Context, conf *system.InitDB) (context.Context, error) // 建库，失败属于 fatal error，因此让它 panic
...
func (s *InitDBService) InitDB(conf system.InitDB) (err error) {
	ctx := context.TODO()
	...
	switch conf.DBType {
	case "mysql":
		initHandler = NewMysqlInitHandler(s.db)
		ctx = context.WithValue(ctx, "dbtype", "mysql")
	case "pgsql":
		initHandler = NewPgsqlInitHandler(s.db)
		ctx = context.WithValue(ctx, "dbtype", "pgsql")
	case "sqlite":
		initHandler = NewSqliteInitHandler(s.db)
		ctx = context.WithValue(ctx, "dbtype", "sqlite")
	case "mssql":
		initHandler = NewMssqlInitHandler(s.db)
