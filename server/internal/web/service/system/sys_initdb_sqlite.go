...
func NewSqliteInitHandler(db *gorm.DB) *SqliteInitHandler {
...
func (h SqliteInitHandler) WriteConfig(ctx context.Context) error {
	c, ok := ctx.Value("config").(orm.Sqlite)
	if !ok {
...
func (h SqliteInitHandler) EnsureDB(ctx context.Context, conf *system.InitDB) (next context.Context, err error) {
