...
func NewMssqlInitHandler(db *gorm.DB) *MssqlInitHandler {
...
func (h MssqlInitHandler) WriteConfig(ctx context.Context) error {
	c, ok := ctx.Value("config").(orm.Mssql)
	if !ok {
...
func (h MssqlInitHandler) EnsureDB(ctx context.Context, conf *system.InitDB) (next context.Context, err error) {
