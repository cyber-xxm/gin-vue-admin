...
func NewAutoCodePluginService(db *gorm.DB) *AutoCodePluginService {
	return &AutoCodePluginService{
		db: db,
	}
}

type AutoCodePluginService struct {
	db *gorm.DB
}

// Install 插件安装
func (s *AutoCodePluginService) Install(file *multipart.FileHeader) (web, server int, err error) {
...
func (s *AutoCodePluginService) PubPlug(plugName string) (zipPath string, err error) {
	if plugName == "" {
...
func (s *AutoCodePluginService) InitMenu(menuInfo system.InitMenu) (err error) {
	...
	var menus []system2.SysBaseMenu

	parentMenu := []system2.SysBaseMenu{
		{
			ParentId:  0,
	...
	err = s.db.Find(&menus, "id in (?)", menuInfo.Menus).Error
	if err != nil {
		return err
	}
...
func (s *AutoCodePluginService) InitAPI(apiInfo system.InitApi) (err error) {
	...
	var apis []system2.SysApi
	err = s.db.Find(&apis, "id in (?)", apiInfo.APIs).Error
	if err != nil {
		return err
	}
