...
import (
	...
	utilsAst "github.com/cyber-xxm/gin-vue-admin/internal/utils/ast"
	"go/ast"
	"go/format"
	"go/parser"
	"go/token"
...
func NewAutoCodeTemplateService(db *gorm.DB) *AutoCodeTemplateService {
	return &AutoCodeTemplateService{
		db:                       db,
		autocodeHistory:          NewAutoCodeHistoryService(db),
		sysExportTemplateService: NewSysExportTemplateService(db),
		autoCodePackageService:   NewAutoCodePackageService(db),
	}
}

type AutoCodeTemplateService struct {
	db                       *gorm.DB
	autocodeHistory          *AutoCodeHistoryService
	sysExportTemplateService *SysExportTemplateService
	autoCodePackageService   *AutoCodePackageService
}

func (s *AutoCodeTemplateService) checkPackage(Pkg string, template string) (err error) {
	switch template {
	case "package":
...
// Create 创建生成自动化代码
func (s *AutoCodeTemplateService) Create(ctx context.Context, info system.AutoCode) error {
	history := info.History()
	var autoPkg system2.SysAutoCodePackage
	err := s.db.WithContext(ctx).Where("package_name = ?", info.Package).First(&autoPkg).Error
	if err != nil {
	...
	if s.autocodeHistory.Repeat(info.BusinessDB, info.StructName, info.Abbreviation, info.Package) {
	...
	if info.AutoCreateMenuToSql {
		var entity system2.SysBaseMenu
		var id uint
		err := s.db.WithContext(ctx).First(&entity, "name = ?", info.Abbreviation).Error
		if err == nil {
			id = entity.ID
		} else {
			entity = info.Menu(autoPkg.Template)
			if info.AutoCreateBtnAuth && !info.OnlyTemplate {
				entity.MenuBtn = []system2.SysBaseMenuBtn{
				...
				if info.HasExcel {
					excelBtn := []system2.SysBaseMenuBtn{
	...
	if info.HasExcel {
		...
		sysExportTemplate := system2.SysExportTemplate{
			DBName:       dbName,
			Name:         name,
			TableName:    tableName,
			TemplateID:   name,
		...
		err = s.sysExportTemplateService.CreateSysExportTemplate(&sysExportTemplate)
		if err != nil {
			return err
		}
	...
	err = s.autocodeHistory.Create(ctx, history)
	if err != nil {
		return err
	}
	return nil
...
// Preview 预览自动化代码
func (s *AutoCodeTemplateService) Preview(ctx context.Context, info system.AutoCode) (map[string]string, error) {
	var entity system2.SysAutoCodePackage
	err := s.db.WithContext(ctx).Where("package_name = ?", info.Package).First(&entity).Error
	if err != nil {
		return nil, errors.Wrap(err, "查询包失败!")
	}
	// 增加判断: 重复创建struct 或者重复的简称
	if s.autocodeHistory.Repeat(info.BusinessDB, info.StructName, info.Abbreviation, info.Package) && !info.IsAdd {
...
func (s *AutoCodeTemplateService) generate(ctx context.Context, info system.AutoCode, entity system2.SysAutoCodePackage) (map[string]strings.Builder, map[string]string, map[string]utilsAst.Ast, error) {
	templates, asts, _, err := s.autoCodePackageService.templates(ctx, entity, info)
	if err != nil {
...
func (s *AutoCodeTemplateService) AddFunc(info system.AutoFunc) error {
	autoPkg := system2.SysAutoCodePackage{}
	err := s.db.First(&autoPkg, "package_name = ?", info.Package).Error
	if err != nil {
		return err
	}
...
func (s *AutoCodeTemplateService) GetApiAndServer(info system.AutoFunc) (map[string]string, error) {
	autoPkg := system2.SysAutoCodePackage{}
	err := s.db.First(&autoPkg, "package_name = ?", info.Package).Error
	if err != nil {
		return nil, err
...
func (s *AutoCodeTemplateService) getTemplateStr(t string, info system.AutoFunc) (string, error) {
...
func (s *AutoCodeTemplateService) addTemplateToAst(t string, info system.AutoFunc) error {
...
func (s *AutoCodeTemplateService) addTemplateToFile(t string, info system.AutoFunc) error {
