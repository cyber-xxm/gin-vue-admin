...
func GormOracle(dbType string, m Oracle) *gorm.DB {
	if m.Dbname == "" {
		return nil
	}
	...
	if db, err := gorm.Open(mysql.New(oracleConfig), Config(dbType, m.Prefix, m.Singular)); err != nil {
		panic(err)
	} else {
		sqlDB, _ := db.DB()
...
func GormOracleByConfig(dbType string, m Oracle) *gorm.DB {
	if m.Dbname == "" {
		return nil
	}
	oracleConfig := mysql.Config{
