...
type GeneralDB struct {
	...
	LogMode      string `mapstructure:"zap-logger-mode" json:"zap-logger-mode" yaml:"zap-logger-mode"` // 是否开启Gorm全局日志
	MaxIdleConns int    `mapstructure:"max-idle-conns" json:"max-idle-conns" yaml:"max-idle-conns"`    // 空闲中的最大连接数
	MaxOpenConns int    `mapstructure:"max-open-conns" json:"max-open-conns" yaml:"max-open-conns"`    // 打开到数据库的最大连接数
	Singular     bool   `mapstructure:"singular" json:"singular" yaml:"singular"`                      // 是否开启全局禁用复数，true表示开启
	LogZap       bool   `mapstructure:"zap-logger-zap" json:"zap-logger-zap" yaml:"zap-logger-zap"`    // 是否通过zap写入日志文件
}
...
func Config(dbType, prefix string, singular bool) *gorm.Config {
	var general GeneralDB
	switch dbType {
	case "mysql":
	case "pgsql":
	case "oracle":
